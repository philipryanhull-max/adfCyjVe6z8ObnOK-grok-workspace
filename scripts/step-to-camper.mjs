...
const src = "/workspace/public/models/camper-layout.step";
...
function classifyName(name) {
  ...
  if (n.includes("top cap")) return 1;
  if (n.includes("hatch door") || n.includes("back hatch")) return 2;
  return 0;
}
...
log(`meshes base=${counts[0]} cap=${counts[1]} hatches=${counts[2]}`);
...
for (let i = 0; i < result.meshes.length; i++) {
  ...
  // As-built powder coat (photos). CAD ships aluminum satin.
  const c =
    group === 1
      ? [0.16, 0.16, 0.165]
      : group === 2
...
writeFileSync(`${outDir}/camper-topper.bin`, Buffer.from(out));
writeFileSync(`${outDir}/camper-topper.bin.gz`, gz);
const byG = [0, 0, 0];
...
log(
  JSON.stringify({
    ...
    verts: { base: byG[0], cap: byG[1], hatches: byG[2] },
    envelopeIn: {
      x: +(hx * 2).toFixed(2),
      y: +(hy * 2).toFixed(2),
      z: +(hz * 2).toFixed(2),
