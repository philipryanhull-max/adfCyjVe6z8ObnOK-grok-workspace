...
const src = "/workspace/public/models/50-BRK-480.step";
...
function classifyName(name) {
  ...
  if (n.includes("12 ga breaker")) return 2;
  if (n.includes("14 ga steel cladding") || n.includes("14 ga steel")) return 1;
  if (n.includes("11 ga")) return 1;
  if (n.includes("steel framing assembly")) return 0;
  return null;
}
...
log(`meshes frame=${counts[0]} cladding=${counts[1]} panels=${counts[2]}`);
...
writeFileSync(`${outDir}/50-brk-480.bin`, Buffer.from(out));
writeFileSync(`${outDir}/50-brk-480.bin.gz`, gz);
const byG = [0, 0, 0];
...
log(
  JSON.stringify({
    ...
    verts: { frame: byG[0], cladding: byG[1], panels: byG[2] },
  }),
);
