...
const src = "/workspace/public/models/vw-dining-table.step";
...
function classifyName(name) {
  ...
  if (n.includes("vw bus") || n.includes("vw ")) return 2;
  if (n.trim() === "top" || n.startsWith("top ")) return 1;
  return 0;
}
...
log(`meshes base=${counts[0]} top=${counts[1]} bus=${counts[2]}`);
...
for (let i = 0; i < result.meshes.length; i++) {
  ...
  const GRAY = [0.722, 0.741, 0.729];
  let c;
  if (group === 0 || group === 1) {
    c = GRAY;
  } else if (native) {
    const luma = native[0] * 0.2126 + native[1] * 0.7152 + native[2] * 0.0722;
    const isRed = native[0] > 0.5 && native[1] < 0.25;
    const isDark = luma < 0.12;
    c = isRed || isDark ? native : GRAY;
  } else {
    c = GRAY;
  }
...
writeFileSync(`${outDir}/vw-dining-table.bin`, Buffer.from(out));
writeFileSync(`${outDir}/vw-dining-table.bin.gz`, gz);
const byG = [0, 0, 0];
...
log(
  JSON.stringify({
    ...
    verts: { base: byG[0], top: byG[1], bus: byG[2] },
    envelopeIn: {
      x: +(hx * 2).toFixed(2),
      y: +(hy * 2).toFixed(2),
      z: +(hz * 2).toFixed(2),
