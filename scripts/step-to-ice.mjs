...
const src = "/workspace/public/models/ice-maker-transition.step";
...
function classifyName(name) {
  ...
  if (n.includes("1500sm")) return 1;
  if (n.startsWith("sm") || n.includes("roof")) return 2;
  return 0;
}
...
log(`meshes ducts=${counts[0]} ice=${counts[1]} steel=${counts[2]}`);
...
for (let i = 0; i < result.meshes.length; i++) {
  ...
  let c;
  if (group === 1) {
    c = [0.627, 0.843, 0.843]; // ice-maker paint
  } else if (native) {
    c = native;
  } else {
    c = [0.80, 0.80, 0.80];
...
writeFileSync(`${outDir}/ice-maker-transition.bin`, Buffer.from(out));
writeFileSync(`${outDir}/ice-maker-transition.bin.gz`, gz);
const byG = [0, 0, 0];
...
log(
  JSON.stringify({
    ...
    verts: { ducts: byG[0], ice: byG[1], steel: byG[2] },
    envelopeIn: {
      x: +(hx * 2).toFixed(2),
      y: +(hy * 2).toFixed(2),
      z: +(hz * 2).toFixed(2),
