...
const src = "/workspace/public/models/follow-up-lever.step";
...
function classifyName(name) {
  ...
  if (n.includes("steering")) return 2;
  if (n.includes("housing")) return 1;
  return 0;
}
...
log(`meshes base=${counts[0]} housing=${counts[1]} steering=${counts[2]}`);

function longestSpan(m) {
...
function meshColor(m, group) {
  const n = (m.name || "").toLowerCase();
  if (n.includes("handle") || n.includes("57455")) return [0.05, 0.05, 0.05];
  // Handle ships unnamed; 3-7/8 in grip + ferrule sit on the steering shaft.
  if (group === 2 && !n.trim() && longestSpan(m) > 0.6) return [0.05, 0.05, 0.05];
  return m.color ?? [0.72, 0.74, 0.73];
}
...
for (const { group, color, geos } of buckets.values()) {
  ...
  merged.dispose();
  const creased = toCreasedNormals(welded, (42 * Math.PI) / 180);
  welded.dispose();
  const pos = creased.attributes.position.array;
  const nrm = creased.attributes.normal.array;
  const idx = creased.index ? creased.index.array : null;
  const index = idx
...
writeFileSync(`${outDir}/follow-up-lever.bin`, Buffer.from(out));
writeFileSync(`${outDir}/follow-up-lever.bin.gz`, gz);
const byG = [0, 0, 0];
...
log(
  JSON.stringify({
    ...
    verts: { base: byG[0], housing: byG[1], steering: byG[2] },
  }),
);
